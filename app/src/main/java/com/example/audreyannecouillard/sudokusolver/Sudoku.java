...
class Sudoku {
    ...
    //This constructor takes an Array of 81 elements as input
    ...
    int[] solveSudoku() {
        ...
        while (isNotSolved(sudoku)) {
            ...
            //createHints can return a null array if the sudoku is not solvable
            int[][] hints = createHints(sudoku);
            if(hints==null){
                //we will return null if the sudoku is not solvable
                return null;
            }
            ...
            if (isNotSolved(sudoku)) {
                ...
                if (isNotSolved(sudoku)) {
                    hints = createHints(sudoku);
                    //we need to check again if the method returned null
                    if(hints==null){
                        return null;
                    }
                    hints=cleanUpHints(hints);
        ...
        }// Once we are done, we return the solved sudoku
        return sudoku;
    }
    //this method checks if the grid is empty (the array is all zeros)
    boolean isSudokuEmpty(){
        for (int element : this.sudokuArray) {
            if (element != 0) {
                return false;
            }
        }
        // if we did not return in the loop then all the element are 0 and the sudoku is empty
        return true;
    }
    //this method checks if the sudoku is solved
    ...
    private static int[][] createHints(int[] sudoku) {
        ...
        for (int i = 0; i < 81; i++) {
            if (sudoku[i] == 0) {
                ...
                //partial hint can return a null array if the sudoku is not solvable, we want to check if that is the case
                if(line==null||column==null||square==null){
                    //we will again return a null array and deal with this in solve sudoku
                    return null;
                }
    ...
    private static int[] partialHint(int[] array) {
        ...
        int count = 0;
        // this is where we check if the sudoku entered was correctly entered
        ...
        if (isArrayLegal(array)) {
         ...
         //if the sudoku is not correctly entered we return an array which is null
        }else{
            return null;
        }
        return partialHint;
